...
class Player:
    ...
    def printTiles(self): print(self.name + "'s Tiles:"); [print('\t{}:{}{}'.format(key, "\t", self.tiles[key])) for key in self.tiles]
    def addTileToPlayerTiles(self,color,number):self.sort(self.tiles[color][number in self.tiles[color][0]].append(number))
    def assignTiles(self,tilesDeck):[self.assignTile(tilesDeck) for i in range(14)]
    def assignTilesRec(self,tilesDeck,numOfTiles):return self.assignTilesRec(self.assignTile(tilesDeck),numOfTiles-1) if numOfTiles>0 else None
    def placeTileOnTable(self, color, number):return self.tiles[color][number in self.tiles[color][1]].remove(number) if any(number in sub for sub in self.tiles[color]) else print(self.errorMessages[1])
    def checkIfTileExists(self,color,number)->bool:return any(number in sub for sub in self.tiles[color])
    def checkDuplicateTiles(self,tiles)->bool:return len(set(list(zip(tiles[0::2],tiles[1::2]))))!=len(tiles[0::2])
    def areAllTilesSameColor(self,tiles)->bool:return len(set(tiles[0::2]))==1
    def areAllTilesSameColorRec(self,tiles,colorSet:set)->bool:return len(colorSet)==1 if len(tiles)<1 else self.areAllTilesSameColorRec(tiles[2:],colorSet|{tiles[0]}) 
    def getOnlyTileNumbersRec(self,tiles,tileNums:list)->list:return tileNums if len(tiles)==0 else self.getOnlyTileNumbersRec(tiles[2:],tileNums+[tiles[1]]) 
    def areAllTilesSameNumber(self,tilesNum)->bool:return len(set(tilesNum))==1
    def areAllTilesSameNumberRec(self,tilesNum)->bool:return True if len(tilesNum)==0 else tilesNum[0]==tilesNum[-1] and self.areAllTilesSameNumberRec(tilesNum[1:-1])
    def isValidSequence(self,tilesNum):return all(tilesNum[i+1]-tilesNum[i]==1 for i in range(len(tilesNum)-1))
    def isValidSequenceRec(self,tilesNum:list)->bool:return True if len(tilesNum)<2 else tilesNum[0]==tilesNum[1]-1 and self.isValidSequenceRec(tilesNum[1:])
    ...
    def assignTile(self,tilesDeck):
        ...
        return tilesDeck
    def sort(self,*args): {index.sort() for sub in self.tiles for index in self.tiles[sub]}